...
abstract class Cannon extends Piece {
     ...
     * @param x 
     * @param y 
     * @returns boolean to indicate if the pass in coordinate is valid Cannon position
     */
    protected abstract isValidCannonPosition(x: number, y: number): boolean;


    /**
     * canMove(): a function that deals with destination check
     * - if the destination is not avaliable, it returns false 
     * @param {dest}: of Type Point}
     * 
     * 
     */
    public canMove(dest: Point): boolean {
        ...
        //If the place is outside of reach from the Cannon,
        //It returns false.
